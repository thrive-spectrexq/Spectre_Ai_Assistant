...
if not st.session_state.api_key:
    st.title("Welcome to Spectre AI Assistant")
    st.session_state.api_key = st.text_input("Please enter your Gemini Pro API Key:", type="password")
    if st.button("Submit API Key") and st.session_state.api_key:
        st.success("API Key set successfully. You can now ask questions.")
else:
    # Set Up Google Gemini with the user-provided API key
    llm = ChatGoogleGenerativeAI(model="gemini-pro", api_key=st.session_state.api_key)
    ...
    def get_response(prompt):
        ...
        messages = [{"role": "system", "content": "You are a helpful and friendly assistant."}] + st.session_state.conversation_history
    ...
    if input_method == "Type":
        ...
        if st.button("Submit"):
            if prompt:
                ...
                logging.info(f"User prompt: {prompt} | AI response: {response} | Emotion: {emotion}")
